...
func (m *Manager) Check(ctx context.Context) (*Release, error) {
	...
	if err := validateDigest(digest); err != nil {
		return nil, fmt.Errorf("update check invalid digest: %w", err)
	}
...
func (m *Manager) DownloadRelease(ctx context.Context, release *Release, currentPath string) error {
	...
	if err := validateDigest(release.Digest); err != nil {
	...
	f, err := os.OpenFile(tmpPath, os.O_CREATE|os.O_TRUNC|os.O_WRONLY, 0o755) // #nosec G302,G304 -- staged update binary path is adjacent to the current executable and must be executable.
	if err != nil {
		return err
	}
	...
	if err := os.Chmod(tmpPath, 0o755); err != nil { // #nosec G302 -- staged update binary must be executable before smoke testing.
		_ = os.Remove(tmpPath)
		return err
	}
...
func RestartProcess(currentPath string) error {
	return syscall.Exec(currentPath, os.Args, os.Environ()) // #nosec G204,G702 -- updater restarts the current executable path directly without shell expansion.
}
...
func validateDigest(digest string) error {
	...
	if len(strings.TrimSpace(expected)) != sha256.Size*2 {
		return fmt.Errorf("invalid sha256 digest length %d", len(strings.TrimSpace(expected)))
	}
	if _, err := hex.DecodeString(strings.TrimSpace(expected)); err != nil {
		return fmt.Errorf("invalid sha256 digest hex: %w", err)
	}
	return nil
}
...
func fileDigest(path string, h hash.Hash) (string, error) {
	f, err := os.Open(path) // #nosec G304 -- path is a staged update file selected by the updater flow.
	if err != nil {
		return "", err
	}
	defer func() {
...
func smokeTestBinary(binaryPath string) error {
	...
	cmd := exec.CommandContext(smokeCtx, binaryPath, "diagnose", "status") // #nosec G204 -- binary is digest-verified before this smoke test.
	cmd.Env = os.Environ()
...
func copyFile(src, dst string) error {
	in, err := os.Open(src) // #nosec G304 -- updater copies the current executable to a sibling backup path.
	if err != nil {
		return err
	}
	defer func() {
	...
	out, err := os.OpenFile(dst, os.O_CREATE|os.O_TRUNC|os.O_WRONLY, info.Mode()) // #nosec G304 -- updater writes a sibling backup for the current executable.
	if err != nil {
		return err
	}
	defer func() {
